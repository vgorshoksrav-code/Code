...
async def duel_action(data: dict):
    ...
    # Регенерация энергии каждый раз при запросе (даже при поллинге)
    match.regenerate_energy()

    if skill_index is None:
        # Просто запрос состояния
    ...
    if match.is_finished():
        ...
        # Начисление наград и ELO
        if match.player1_id != -1 and match.player2_id != -1:
            # Оба игрока – люди
            update_elo(winner, loser)
        else:
            # Бой с AI: только осколки и статистика, без изменения рейтинга
...
async def buy_skin(data: dict):
    ...
    if new_stars >= 5000 and user['is_vip'] == 0:
        ...
        add_skin_to_user(user_id, 3)  # VIP скин
    conn.commit()
    conn.close()
